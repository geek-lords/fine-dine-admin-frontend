                ...
                if (tables.hasOwnProperty(key)) {
                    ...
                    <div class="xl:w-1/3 md:w-1/2 p-4 new_order_cards">
                        <div class="bg-gray-100 p-6 rounded-lg shadow-md border-2">
                            ...
                            <div id="${table.replace(/\s/g, "")}_menu_div" class=" flex flex-col my-3">                                
                            </div>
                        </div>
                    </div>
                    `)
                    orders.forEach(element => {
                       ...
                       <div id="item_${element["id"]}" class="flex flex-rowborder-2 border border-indigo-400 h-auto rounded-xl shadow-sm">
                       <p class="p-3 w-5/6">${element["name"]}</p>
                       <input id="${element["id"]}" onclick="newItemChecked(this)" type="checkbox" class="mt-4">
                       <p class="mt-2 ml-1 mb-2  align-middle bg-indigo-500 px-2 rounded-md text-white">${element["quantity"]}</p>
                       </div>
                       `)
                    });
                ...
                for (let key in tables) {
                    ...
                    <div class="xl:w-1/3 md:w-1/2 p-4 served_order_cards">
                        <div class="bg-gray-100 p-6 rounded-lg shadow-md border-2">
                            ...
                            <p class="text-xs mt-2">Cust Name:<span class="ml-2 text-purple-600">${username}</span></p>
                            <div id="${table.replace(/\s/g, "")}_served_menu_div" class=" flex flex-col my-3">                                
                            </div>
                        </div>
                    </div>
                    `)
                    orders.forEach(element => {
                       ...
                       <div class="flex flex-rowborder-2 border border-indigo-400 h-auto rounded-xl shadow-sm">
                       <p class="p-3 w-5/6">${element["menu.name"]}</p>
                       <p class="m-4 float-right align-middle bg-indigo-500 px-2 rounded-md text-white">${element["quantity"]}</p>
                       `)
                    });
